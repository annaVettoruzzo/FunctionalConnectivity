...
%tasks relating FC values with ZCR feature extracted from EMG data.
%
...
%subject, the average ZC in 10-Motort, the average ZC in 11-Motort
...
for i=1:length(subjects)
    ...
    EMG_data{i,2} = mean(zc_v); %average all ZC
...
%average the ZC from the two runs
...
if selectBand == "True"
    if ntwName == "CON"
        folderName = 'D:\Tesi\Codice\EMG_analysis\ResultsEMG\ZCR\FirstApproach\bandLimited/CONT/'; 
    else
        folderName = ['D:\Tesi\Codice\EMG_analysis\ResultsEMG\ZCR\FirstApproach\bandLimited/', ntwName, '/'];
    end
else
    if ntwName == "CON"
        folderName = 'D:\Tesi\Codice\EMG_analysis\ResultsEMG\ZCR\FirstApproach/CONT/'; 
    else
        folderName = ['D:\Tesi\Codice\EMG_analysis\ResultsEMG\ZCR\FirstApproach/', ntwName, '/'];
    end
end
...
fprintf(fileID, '± %3.3e \n\n', std_diag);
%IDrate
fprintf(fileID, 'Identification rate: %3.3e%%\n\n', IDrate);
%Min value IDmatrix
